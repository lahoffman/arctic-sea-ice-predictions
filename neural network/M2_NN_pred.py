...
for i in range(12):
    ...
    for j in range(1):
        ...
        for h in range(1):
            ...
            model.summary()
            savepath = filepath+'model_2DNN_siesia2sie_'+time_frames[i]+'_tau'+str(h)+'_T'+str(j)+'.h5'
            model.save(savepath)
            ...
            #Make the prediction from the model, validation
            predictions = model.predict(x_val)
            pred_expectedval =  np.dot(predictions,binCenters)
            true_expectedval = np.dot(y_val,binCenters)

            plt.figure(figsize=(12,4))
            plt.subplot(1,2,1)
            plt.plot(true_expectedval, pred_expectedval,'ro')
            plt.plot(true_expectedval,true_expectedval ,'k')
            plt.xlabel('True ')
            plt.ylabel('Predicted ')
            plt.title('Network-Predicted forced response, validation') 
            plt.axis('tight')
            plt.show()
            ...
            #Make the prediction from the model, obs
            ...
            #Make the prediction from the model, train
            predictions = model.predict(x_train)
            pred_expectedtr =  np.dot(predictions,binCenters)
            true_expectedtr = np.dot(y_train,binCenters)
            ...
            plt.xlabel('True ')
            plt.ylabel('Predicted ')
            plt.title('Network-Predicted forced response, train') 
            plt.axis('tight')
            plt.show()
        
            savepath = filepath+'prediction_output_'+time_frames[i]+'_tau'+str(h)+'_T'+str(j)+'.npz'
            np.savez(savepath, pred_tr = pred_expectedtr, true_tr = true_expectedtr,pred_va = pred_expectedval, true_va = true_expectedval,pred_te = pred_expectedte, true_te = true_expectedte,pred_obs = pred_expectedobs, true_obs = true_expectedobs, pred = predictions)

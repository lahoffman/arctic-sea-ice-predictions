...
#------------------------------------------------------    
for z in range(1): #ALL
#for z in range(6): #INDIVIDUAL

    #select specific CMIP6 model, change ensemble_no_index index
    #test_data = sie_training[:,ensemble_no_index[z]:ensemble_no_index[z+1]-1,:,:] #INDIVIDUAL
    test_data = sie_training #ALL
    ...
    to_cod_CI_m.append(to_cod_CI)
    to_rel_CI_m.append(to_rel_CI)
    to_cod_m.append(to_cod)
    to_rel_m.append(to_rel)
...
oneyearjulyTO_cod_CI = np.array(to_cod_CI[:,6,0,0,:])
oneyearjulyTO_rel_CI = np.array(to_rel_CI[:,6,0,0,:])
...
to_cod_CI = np.array(to_cod_CI_m)
to_rel_CI = np.array(to_rel_CI_m)
to_cod = np.array(to_cod_m)
to_rel = np.array(to_rel_m)
    
#savepath = '/Users/hoffmanl/Documents/data/manuscripts/ms1/d3/TO_sie2sie_performance_perfect_CMIP6_CI.nc'
savepath = '/Users/hoffmanl/Documents/data/manuscripts/ms1/d3/TO_sie2sie_performance_perfect_CMIP6_ALL_CI.nc'
with nc.Dataset(savepath,'w') as file:
    #create dimensions
    #file.createDimension('nm',percentskillfulTO5.shape[0]) #no. models
    file.createDimension('nm',1) #use for ALL
    
    file.createDimension('tf',to_cod_CI.shape[1]) #no. time frames
    file.createDimension('T',to_cod_CI.shape[2]) #no. moving means
    file.createDimension('tau',to_cod_CI.shape[3]) #no lag times
    file.createDimension('ci',to_cod_CI.shape[4]) #no. CI bounds
    ...
    codTO = file.createVariable('to_cod','f4',('nm','tf','T','tau'))
    relTO = file.createVariable('to_rel','f4',('nm','tf','T','tau'))
    codTOCI = file.createVariable('to_cod_CI','f4',('nm','tf','T','tau','ci'))
    relTOCI = file.createVariable('to_rel_CI','f4',('nm','tf','T','tau','ci'))
